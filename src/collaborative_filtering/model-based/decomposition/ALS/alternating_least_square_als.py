...
from utils.load_data import load_movie_ratings
from utils.utils import run_time


class ALS(object):
    """ALternative Least Square(ALS) class.
    ...
    Args:
        ...
        shape (tuple) -- Dimension of rating matrix.
        rmse (float) -- Square root of mse, (Sum((R - U_T * I)) ^ 2 / n_elements) ^ 0.5
    """
    def __init__(self):
        self.user_ids = None
        self.item_ids = None
    ...
    def _process_data(self, X):
        ...
        # Shape of item rating data matrix
    ...
    def _users_mul_ratings(self, users, ratings_T):
        ...
        Args:
            ...
            ratings_T (dict): The items ratings by users. {item_id: {user_id: rating}}
        Returns:
            Matrix: Item matrix.
        """
    ...
    def _items_mul_ratings(self, items, ratings):
        ...
        The result(users) is a k * n matrix, n stands for number of user_ids.
        Args:
            items (Matrix): k * n matrix, n stands for number of item_ids.
            ratings (dict): The items ratings by users. {user_id: {items_id: rating}}
        Returns:
            Matrix: User matrix.
        """
    ...
    def _gen_random_matrix(self, n_rows, n_colums):
        """
        Generate a n_rows * n_columns matrix with random number.
        Args:
    ...
    def fit(self, X, k, max_iter = 10):
        """
        Build an ALS model.
            Suppose the rating matrix R can be decomposed as U * I, U stands for User and I stands for Item:
                R(m, n) = U(k, m)_transpose * I(k, n)
            Use MSE as loss function:
                Loss(U, I) = sum((R_ij - U_i_transpose * I_j) ^ 2)
            Take the partial of the function:
                dLoss(U, I) / dU = -2 * sum(I_j * (R_ij - U_i_transpose * I_j)_transpose)
        ...
        m, n = self.shape
        err_msg = "Parameter k must be less than the rank of original matrix"
        assert k < min(m, n), err_msg

        # Initialize users and item matrix
        ...
        for i in range(max_iter):
            if i % 2:
                ...
                self.user_matrix = self._items_mul_ratings(items.mat_mul(items.transpose).inverse.mat_mul(items), ratings)
            else:
                # I = (U * U_transpose) ^ (-1) * U * R
                users = self.user_matrix
                self.item_matrix = self._users_mul_ratings(users.mat_mul(users.transpose).inverse.mat_mul(users), ratings_T)
...
def main():
    ...
    # ------------------------------------------
    print("Testing the performance of ALS...")
    # ------------------------------------------
    # Train model
    model = ALS()
    ...
    for user_id, prediction in zip(user_ids, predictions):
        _prediction = ["item_id%d score:%.2f" % (item_id, score) for item_id, score in prediction]
